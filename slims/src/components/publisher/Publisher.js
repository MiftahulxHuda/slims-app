...
export default class Publisher extends Component {
    ...
    async getData() {
        const list = await CRUDService.findAll("/mst-publisher", {
            take: this.take,
            skip: this.state.currentPage,
            sort: "publisher_name,publisher_id",
            ...this.props.filter
        })
    ...
    renderItem({ item }) {
        return (
            <PublisherItem
                ...
                onEdit={() => this.props.navigation.push("FormPublisher", { action: "edit", id: item.publisher_id })}
                onDelete={() => {
                    this.setState({ deleteId: item.publisher_id })
            ...
            <>
                ...
                >
                    <ModalDelete
                        ...
                        onSubmit={async () => {
                            const deleted = await CRUDService.deleteOneById("/mst-publisher", this.state.deleteId);
                            if (deleted) {
                                const filtered = await this.state.data.filter(filter => filter.publisher_id != this.state.deleteId);
                ...
                <View style={styles.container}>
                    <Header title="Publisher"
                        ...
                        onPressFilter={() => {
                            this.props.navigation.push("FilterPublisher")
                        }}
                        onAdd={() => this.props.navigation.push("FormPublisher", { action: "add" })}
