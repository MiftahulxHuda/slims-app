...
class MyForm extends PureComponent {
    ...
    async findOneById(id) {
        const findOneById = await CRUDService.findOneById(`${"/mst-media-type"}/${id}`);
        if (findOneById) {
            ...
            <>
                <View style={styles.content}>
                    ...
                    <CustomTextInput
                        label="Media Type"
                        required={true}
                        onChangeText={handleChange('media_type')}
                        onBlur={handleBlur('media_type')}
...
const create = async (post) => {
    const created = await CRUDService.create("/mst-media-type", post);
    return created;
}

const updateOneById = async (id, post) => {
    const updated = await CRUDService.updateOneById("/mst-media-type", id, post);
    return updated;
}

const formikEnhancer = withFormik({
    mapPropsToValues: (props) => {
        return ({ code: "", code2: "", media_type: "" })
    },
