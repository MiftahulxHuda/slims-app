...
export default class Location extends Component {
    ...
    async getData() {
        const list = await CRUDService.findAll("/mst-location", {
            take: this.take,
            skip: this.state.currentPage,
            sort: "location_name,location_id",
            ...this.props.filter
        })
    ...
    renderItem({ item }) {
        return (
            <LocationItem
                ...
                onEdit={() => this.props.navigation.push("FormLocation", { action: "edit", id: item.location_id })}
                onDelete={() => {
                    this.setState({ deleteId: item.location_id })
            ...
            <>
                ...
                >
                    <ModalDelete
                        ...
                        onSubmit={async () => {
                            const deleted = await CRUDService.deleteOneById("/mst-location", this.state.deleteId);
                            if (deleted) {
                                const filtered = await this.state.data.filter(filter => filter.location_id != this.state.deleteId);
                ...
                <View style={styles.container}>
                    <Header title="Location"
                        ...
                        onPressFilter={() => {
                            this.props.navigation.push("FilterLocation")
                        }}
                        onAdd={() => this.props.navigation.push("FormLocation", { action: "add" })}
