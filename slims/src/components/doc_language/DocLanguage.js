...
export default class DocLanguage extends Component {
    constructor(props) {
        super(props)
    ...
    async getData() {
        const list = await CRUDService.findAll("/mst-language", {
            take: this.take,
    ...
    renderItem({ item }) {
        return (
            <DocLanguageItem
                ...
                onEdit={() => this.props.navigation.push("FormDocLanguage", { action: "edit", id: item.language_id })}
                onDelete={() => {
                    this.setState({ deleteId: item.language_id })
            ...
            <>
                ...
                >
                    <ModalDelete
                        ...
                        onSubmit={async () => {
                            const deleted = await CRUDService.deleteOneById("/mst-language", this.state.deleteId);
                            if (deleted) {
                                const filtered = await this.state.data.filter(filter => filter.language_id != this.state.deleteId);
                ...
                <View style={styles.container}>
                    <Header title="Doc Language"
                        ...
                        onPressFilter={() => {
                            this.props.navigation.push("FilterDocLanguage")
                        }}
                        onAdd={() => this.props.navigation.push("FormDocLanguage", { action: "add" })}
