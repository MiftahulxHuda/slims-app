...
export default class Place extends Component {
    ...
    async getData() {
        const list = await CRUDService.findAll("/mst-place", {
            take: this.take,
            skip: this.state.currentPage,
            sort: "place_name,place_id",
            ...this.props.filter
        })
    ...
    renderItem({ item }) {
        return (
            <PlaceItem
                ...
                onEdit={() => this.props.navigation.push("FormPlace", { action: "edit", id: item.place_id })}
                onDelete={() => {
                    this.setState({ deleteId: item.place_id })
            ...
            <>
                ...
                >
                    <ModalDelete
                        ...
                        onSubmit={async () => {
                            const deleted = await CRUDService.deleteOneById("/mst-place", this.state.deleteId);
                            if (deleted) {
                                const filtered = await this.state.data.filter(filter => filter.place_id != this.state.deleteId);
                ...
                <View style={styles.container}>
                    <Header title="Place"
                        ...
                        onPressFilter={() => {
                            this.props.navigation.push("FilterPlace")
                        }}
                        onAdd={() => this.props.navigation.push("FormPlace", { action: "add" })}
