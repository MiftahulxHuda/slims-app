...
export default class ContentType extends Component {
    ...
    async getData() {
        const list = await CRUDService.findAll("/mst-content-type", {
            take: this.take,
            skip: this.state.currentPage,
            sort: "content_type,id",
            ...this.props.filter
        })
    ...
    renderItem({ item }) {
        return (
            <ContentTypeItem
                ...
                onEdit={() => this.props.navigation.push("FormContentType", { action: "edit", id: item.id })}
                onDelete={() => {
            ...
            <>
                ...
                >
                    <ModalDelete
                        ...
                        onSubmit={async () => {
                            const deleted = await CRUDService.deleteOneById("/mst-content-type", this.state.deleteId);
                            if (deleted) {
                ...
                <View style={styles.container}>
                    <Header title="Content Type"
                        onBack={() => this.props.navigation.toggleDrawer()}
                        onPressFilter={() => {
                            this.props.navigation.push("FilterContentType")
                        }}
                        onAdd={() => this.props.navigation.push("FormContentType", { action: "add" })}
