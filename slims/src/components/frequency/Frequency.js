...
export default class Frequency extends Component {
    ...
    async getData() {
        const list = await CRUDService.findAll("/mst-frequency", {
            take: this.take,
            skip: this.state.currentPage,
            sort: "frequency,frequency_id",
            ...this.props.filter
        })
    ...
    renderItem({ item }) {
        return (
            <FrequencyItem
                ...
                time_increment={item.time_increment}
                time_unit={item.time_unit}
                onEdit={() => this.props.navigation.push("FormFrequency", { action: "edit", id: item.frequency_id })}
                onDelete={() => {
                    this.setState({ deleteId: item.frequency_id })
            ...
            <>
                ...
                >
                    <ModalDelete
                        ...
                        onSubmit={async () => {
                            const deleted = await CRUDService.deleteOneById("/mst-frequency", this.state.deleteId);
                            if (deleted) {
                                const filtered = await this.state.data.filter(filter => filter.frequency_id != this.state.deleteId);
                ...
                <View style={styles.container}>
                    <Header title="Frequency"
                        ...
                        onPressFilter={() => {
                            this.props.navigation.push("FilterFrequency")
                        }}
                        onAdd={() => this.props.navigation.push("FormFrequency", { action: "add" })}
                    ...
                    <FlatList
                        ...
                        keyExtractor={item => item.frequency_id}
